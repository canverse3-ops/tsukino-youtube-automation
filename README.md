...
現時点では、画像生成やYouTube投稿は実装していません。後からGemini API、Imagen API、YouTube APIを接続しやすいように、台本解析、シーン生成、画像生成専用プロンプト生成、出力処理を分けています。
...
このリポジトリでは、`01_script/script.md` または `src/main.py` をPushしたときにGitHub Actionsで `python src/main.py` を自動実行し、`02_output/` 以下の制作ファイルを更新できます。
