...
現時点では、PNGやJPGなどの画像生成やYouTube投稿は実装していません。後からGemini API、Imagen API、YouTube APIを接続しやすいように、台本解析、シーン生成、画像生成専用プロンプト生成、出力処理を分けています。画像保存先として `03_images/` を用意しますが、生成されるのは `.gitkeep` のみです。
...
各シーンに対応する制作補助用のプロンプト情報を保存します。安全な最小構成として、台本から決定できる値と共通設定を組み合わせて生成します。

### `image_prompts.json`

`scene.json` の各シーンを元に、画像生成専用の最小項目だけを保存します。画像ファイル名は `scene_01.png`、`scene_02.png`、`scene_03.png` のように連番で生成します。
...
| `narration` | `scene.json` と同じシーン本文 |
| `image_prompt_ja` | 日本語の画像生成プロンプト。ナレーション、日常のワンシーン、静かな違和感、正体暴き、映画の1カットのような構図、感情が伝わる表情や仕草を含む |
...
`image_prompts.json` から、画像生成の実行計画として必要な最小項目だけを保存します。PNGやJPGなどの画像ファイルは生成しません。

| 項目 | 説明 |
| --- | --- |
| `scene_number` | 1から始まるシーン番号 |
| `image_filename` | 将来 `03_images/` に保存する想定の画像ファイル名。例: `scene_01.png` |
| `image_prompt_ja` | 日本語の画像生成プロンプト |

### `03_images/`

将来の画像出力先として空ディレクトリを管理するため、`03_images/.gitkeep` だけを配置します。このリポジトリの自動生成処理は画像ファイルを作成しません。
...
- Imagen API: `image_prompts.json` を使った画像生成
...
このリポジトリでは、`01_script/script.md` または `src/main.py` をPushしたときにGitHub Actionsで `python src/main.py` を自動実行し、`02_output/` 以下の制作ファイルを更新できます。`03_images/.gitkeep` はディレクトリ維持用で、ActionsでもPNGやJPGなどの画像ファイルは生成しません。

### 自動生成されるファイル

- `02_output/scene.json`
- `02_output/prompts.json`
- `02_output/image_prompts.json`
- `02_output/image_generation_plan.json`
...
ワークフローはPush時に実行されますが、対象パスを `01_script/script.md`、`src/main.py`、ワークフローファイルに限定しています。GitHub Actionsが `02_output/` だけを自動コミットした場合は再実行されないため、生成コミットによる無限ループを防ぎます。加えて、実行条件で `github-actions[bot]` によるPushを除外しています。
